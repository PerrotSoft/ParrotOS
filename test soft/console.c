...
void PrintChar(CHAR16 c, uint32_t fg, uint32_t bg) {
    ...
    if (c == 0x08 || c == 127) { // Правильный Бэкспейс
        if (cursor_x > 0) {
            cursor_x--;
        } else if (cursor_y > 0) {
            cursor_y--;
...
void RenderConsole() {
    ...
    for (int y = 0; y < console_rows; y++) {
        for (int x = 0; x < console_cols; x++) {
            ...
            // 1. Рисуем заливку фона клетки (если она не черная)
            ...
            // 2. Рисуем ОДИН символ строго в своей клетке
            if (cell.Char != 0 && cell.Char != (CHAR16)' ') {
                CHAR16 buf[2] = { cell.Char, 0 }; // Строка из одного символа
                GfxPrint(OFFSET_X + (x * char_w), OFFSET_Y + (y * char_h), font_size, cell.FG, buf);
            }
        }
    }

    // 3. Рисуем курсор. Теперь он ЖЕСТКО привязан к сетке и не сможет "отсоединиться"
    uint32_t cursor_vis_x = OFFSET_X + (cursor_x * char_w);
    uint32_t cursor_vis_y = OFFSET_Y + (cursor_y * char_h) + (char_h - 2); 
    GfxDrawLine(cursor_vis_x, cursor_vis_y, cursor_vis_x + char_w - 1, cursor_vis_y, 0xFFFFFF);
    GfxDrawLine(cursor_vis_x, cursor_vis_y + 1, cursor_vis_x + char_w - 1, cursor_vis_y + 1, 0xFFFFFF);
...
CHAR16* Read() {
    ...
    while (1) {
        ...
        if (c == (CHAR16)'\n' || c == (CHAR16)'\r') {
            ...
            if (index > 0) {
                index--;
                PrintChar(0x08, global_fg, global_bg); // Обработка бэкспейса
                RenderConsole();
            }
        } 
        else if (index < 255) {
