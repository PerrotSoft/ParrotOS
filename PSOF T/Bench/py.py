...
def build():
    ...
    # 1. Компиляция C++ кода
    # Убран ошибочный флаг -fno-wchar
    # Добавлен флаг -fpermissive, чтобы g++ превратил ошибки несовпадения wchar_t/CHAR16 в предупреждения
    res1 = subprocess.run("g++ -ffreestanding -fshort-wchar -fpermissive -m64 -fno-exceptions -fno-rtti -c main.cpp -o main.o", shell=True)

    res2 = subprocess.run("ld -T app.ld main.o -o Main.bin", shell=True)
    ...
    if not build_pex("Main.bin", "usb_root/start.pex"):
        return
    ...
    qemu_cmd = (
        ...
        f"-m 512M "
        f"-drive if=pflash,format=raw,readonly=on,file=/usr/share/ovmf/OVMF.fd " 
        f"-drive format=raw,file=boot.img " 
        f"{accel} "
