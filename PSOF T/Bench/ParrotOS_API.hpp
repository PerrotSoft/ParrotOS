...
namespace ParrotOS {
    ...
    namespace Keys {
        ...
        constexpr uint16_t Right          = 0x0003;
        constexpr uint16_t Left           = 0x0004;
        constexpr uint16_t Esc            = 0x0017;
    }
    ...
    // ==========================================
    namespace Keyboard {
        inline uint64_t GetKey() { uint64_t r; asm volatile ("movq $0x01, %%rax; int $0x22; movq %%rax, %0" : "=r"(r) : : "rax"); return r; }
        inline uint64_t HasKey() { uint64_t r; asm volatile ("movq $0x02, %%rax; int $0x22; movq %%rax, %0" : "=r"(r) : : "rax"); return r; }
        inline void Reset() { asm volatile ("movq $0x03, %%rax; int $0x22" : : : "rax"); }
    }
...
// ==========================================
// ОСНОВНАЯ ТОЧКА ВХОДА (ОБЯЗАТЕЛЬНО EXTERN "C")
// ==========================================

extern "C" void main(ParrotOS::Process* pr); 

extern "C" __attribute__((used, section(".text.boot")))
void _start(ParrotOS::Process* pr) { 
    main(pr); 
