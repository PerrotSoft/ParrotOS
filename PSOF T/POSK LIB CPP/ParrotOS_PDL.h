...
// Размер образа ТЕКУЩЕЙ программы (.pex) в памяти. Требует символ
// __pex_end в конце SECTIONS в app.ld (по аналогии с __pdl_end в pdl.ld).
extern uint8_t __pex_end;
uint64_t SysGetImageSize(void) { return (uint64_t)&__pex_end; }
// ==========================================
...
// ==========================================
// ==========================================
// INT 0x22: KEYBOARD (C-совместимый API)
// ==========================================
static inline CHAR16 KbdGetKeyNonBlocking(void) { 
    register uint64_t rax asm("rax") = 0x01;
    asm volatile ("int $0x22" : "+r"(rax) : : "rcx", "rdx", "rbx", "memory"); 
    return (CHAR16)rax; 
}
    
static inline uint64_t KbdGetBuffer(CHAR16* buffer, uint64_t maxLength) { 
    register uint64_t rax asm("rax") = 0x02;
    register uint64_t rcx asm("rcx") = (uint64_t)buffer;
    register uint64_t rdx asm("rdx") = maxLength;
    asm volatile ("int $0x22" : "+r"(rax), "+r"(rcx), "+r"(rdx) : : "rbx", "memory"); 
    return rax;
}
    
static inline void KbdFlush(void) { 
    register uint64_t rax asm("rax") = 0x03;
    asm volatile ("int $0x22" : "+r"(rax) : : "rcx", "rdx", "rbx", "memory"); 
}
    
static inline CHAR16 KbdGetKey(void) { 
    register uint64_t rax asm("rax") = 0x04;
    asm volatile ("int $0x22" : "+r"(rax) : : "rcx", "rdx", "rbx", "memory"); 
    return (CHAR16)rax;
}
    
static inline BOOL KbdHasKey(void) { 
    register uint64_t rax asm("rax") = 0x05;
    asm volatile ("int $0x22" : "+r"(rax) : : "rcx", "rdx", "rbx", "memory"); 
    return (rax != 0) ? TRUE : FALSE;
}
    
static inline uint64_t KbdGetBufferSize(void) { 
    register uint64_t rax asm("rax") = 0x06;
    asm volatile ("int $0x22" : "+r"(rax) : : "rcx", "rdx", "rbx", "memory"); 
    return rax;
}

static inline void KbdReset(void) { 
...
// Общая статистика по RAM всей ОС. Возвращает 0, если ядро не смогло
// получить карту памяти (out при этом не трогается).
uint64_t MemGetStats(MemoryStats* out) {
    uint64_t r;
    asm volatile (
        "movq $0x03, %%rax\n\t"
        "movq %1, %%rcx\n\t"
