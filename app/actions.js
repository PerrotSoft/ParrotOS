...
export async function updateNewsAlbum(albumId, username, updates = {}) {
  ...
  if (updates.cover !== undefined) fields.cover = String(updates.cover).slice(0, 500);
...
export async function createPost(username, text, image) {
  'use server';
  ...
  try {
    ...
    await client.execute({
      sql: `INSERT INTO wc_posts (id, username, text, image, timestamp) VALUES (?, ?, ?, ?, ?)`,
      args: [id, username, (text || '').trim(), image || '', Date.now()],
    });
...
export async function getPostsFeed(viewerUsername, limit = 50) {
  'use server';
  try {
    await ensureWcPostsTables();
    const rs = await client.execute({ sql: `SELECT * FROM wc_posts ORDER BY timestamp DESC LIMIT ?`, args: [limit] });
    const posts = toPlain(rs.rows);
