...
import { ADSTERRA_GIF_URL, MIN_AD_SECONDS, STATIC_ROTATE_SECONDS, PREROLL_CHANCE, MIDROLL_ROTATE_GIF_CHANCE, pickAdCategory, mergeConfig } from '../../lib/siteConfig';
...
export default function WavyPlayer({ videoId, duration, username, authorAdDevId, authorAdStaticId, authorAdVideoId, adPositions: adPositionsStr, startAt = 0, onTimeUpdate }) {
  ...
  // либо всегда 'banner', без всякой рандомизации. AD_SPLIT в FireSoftAd.jsx
  // на это НИКАК не влиял — это два независимых места выбора рекламы, и
  // именно ЭТО, а не FireSoftAd, отвечает за настоящие мидроллы во время
  ...
  const tryShowMidroll = async (video) => {
    ...
    // выходила — мидроллы не показывались ВООБЩЕ, ни FireSoft, ни Adsterra.
    // У баннеров (FireSoftAd.jsx) уже был дефолт на платформенный аккаунт
    // 'Icfg' — теперь и здесь так же: реклама всегда пытается показаться,
    // getAdForPlayer сам разберётся FireSoft-кампания это или Adsterra.
    const devId = authorAdDevId || 'Icfg';
    const staticId = authorAdStaticId || 'adsterra_default';
    const videoSiteId = authorAdVideoId;

    // Общий разыгрыватель с FireSoftAd.jsx — см. lib/siteConfig.js
    ...
    try {
      ...
      const siteId = category === 'video' ? (videoSiteId || staticId)
        : category === 'bannerAdsterra' ? 'adsterra_default'
        : staticId;
      const reqDevId = category === 'bannerAdsterra' ? 'Icfg' : devId;
  ...
  useEffect(() => {
    ...
    if (adElapsed > 0 && adElapsed % STATIC_ROTATE_SECONDS === 0) {
      ...
      const { authorAdDevId, authorAdStaticId, authorAdVideoId } = adPropsRef.current;
      const devId = authorAdDevId || 'Icfg';
      const siteId = authorAdStaticId || 'adsterra_default';
        ...
        <div style={{ position: 'absolute', inset: 0, zIndex: 50, display: 'flex', flexDirection: 'column', background: 'rgba(0,0,0,0.9)' }}>
          <div style={{ padding: '12px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.95)', color: 'white', fontSize: '15px', fontWeight: 'bold', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <span style={{ color: '#6366f1', fontWeight: 400, fontSize: 13 }}>FireSoft Ads System</span>
              {canSkip ? (
                <button
