...
function PostsFeed({ username, actions, onClose }) {
  const [posts, setPosts] = useState([]);
  const [sortMode, setSortMode] = useState('new'); // 'new' | 'top' — простая "рекомендация": сортировка по свежести или по лайкам
  ...
  const loadFeed = async () => {
    setLoading(true);
    const feed = await actions.getPostsFeed(username);
    setPosts(feed);
    setLoading(false);
  };
  ...
  const submitPost = async () => {
    ...
    setPosting(true);
    const res = await actions.createPost(username, draftText, draftImage);
    setPosting(false);
  ...
  return (
    <div className="wc-posts-overlay">
      <div className="wc-posts-inner">
        <div className="wc-posts-header">
            <div className="wc-mode-switch">
                <button onClick={onClose}>💬 Чаты</button>
                <button className="active">📰 Посты</button>
            </div>
        </div>
        ...
        <div className="wc-posts-sort">
            <button className={sortMode === 'new' ? 'active' : ''} onClick={() => setSortMode('new')}>🕐 Новые</button>
            <button className={sortMode === 'top' ? 'active' : ''} onClick={() => setSortMode('top')}>🔥 Популярное</button>
            <button className={sortMode === 'following' ? 'active' : ''} onClick={() => setSortMode('following')}>👥 Подписки</button>
        </div>
        ...
        {[...posts]
          ...
          .map(p => (
          <div key={p.id} className="wc-post-card">
            <div className="wc-post-header">
              ...
              <div>
                ...
                <div className="wc-post-time">{new Date(p.timestamp).toLocaleString('ru')}</div>
              </div>
              {p.username !== username && (
      ...
      <style jsx>{`
        ...
        .wc-posts-inner { max-width: 560px; margin: 0 auto; }
        .wc-posts-header { display: flex; justify-content: center; margin-bottom: 16px; }
        .wc-posts-header .wc-mode-switch {
          display: flex; gap: 3px; background: var(--wc-surface2, rgba(127,127,127,0.1));
          border: 1px solid var(--wc-border); border-radius: 16px; padding: 3px;
        }
        .wc-posts-header .wc-mode-switch button {
          background: transparent; border: none; color: var(--wc-muted);
          padding: 6px 14px; border-radius: 13px; cursor: pointer; font-size: 12px; font-weight: 600;
        }
        .wc-posts-header .wc-mode-switch button.active { background: var(--wc-accent); color: #fff; }
        .wc-posts-sort {
        ...
        .wc-posts-composer-row { display: flex; justify-content: space-between; align-items: center; margin-top: 10px; }
...
export default function WevyChat() {
    ...
    return (
        <div className={`app theme-${theme} ${fastMode ? 'perf-fast' : ''}`} style={{ '--wc-accent': accent, '--wc-msg-own-bg': `linear-gradient(135deg, ${accent}, ${accent}cc)` }}>
            {viewMode === 'posts' && (
                <PostsFeed username={currentUser} actions={actions} onClose={() => setViewMode('chats')} />
            )}
